...
//   adjusted_events_per_100_miles?: number;
...
//     const per100Miles = analytics.adjusted_events_per_100_miles || 
...
//         <div style={{ fontWeight: '600', color: '#2c3e50' }}>{safeToFixed(trip.adjusted_events_per_100_miles || 0, 2)}</div>
//       </div>
//       <div>
...
// Insurance Provider Dashboard - Privacy-Focused Driver Analytics
import React, { useState, useCallback } from 'react';
import {
  IonPage, IonHeader, IonToolbar, IonTitle, IonContent,
  IonInput, IonButton, IonLabel, IonText, IonCard, IonCardHeader,
...
interface EnhancedTripSummary {
  ...
  formatted_duration?: string;
  avg_speed_mph: number;
  avg_speed_driving_time_only?: number;
  avg_speed_total_time?: number;
  driving_time_minutes?: number;
  stationary_time_minutes?: number;
  driving_time_percentage?: number;
  max_speed_mph: number;
  sudden_accelerations: number;
  sudden_decelerations: number;
  hard_stops: number;
...
const ProviderHome: React.FC<ProviderHomeProps> = ({ user, onSignOut }) => {
  ...
  // Format date and time for trip display
  ...
  // Get properly numbered trips (oldest = Trip 1, newest = Trip N)
  const getNumberedTrips = (trips: EnhancedTripSummary[]) => {
    // Sort trips by start_timestamp (oldest first) for proper numbering
    const sortedTrips = [...trips].sort((a, b) => 
      new Date(a.start_timestamp).getTime() - new Date(b.start_timestamp).getTime()
    );
    
    // Then reverse for display (newest first) but keep the proper numbering
    return trips.map(trip => {
      const tripIndex = sortedTrips.findIndex(t => t.trip_id === trip.trip_id);
      return {
        ...trip,
        tripNumber: tripIndex + 1,  // Trip 1 = oldest, Trip N = newest
        ...
        {analytics && (
          <div style={{ padding: '20px', maxWidth: '1000px', margin: '0 auto' }}>
            ...
            <div style={{
              ...
              {analytics.trips && analytics.trips.length > 0 ? (
                <div>
                  {getNumberedTrips(analytics.trips).slice(0, showDetailedView ? analytics.trips.length : 5).map((trip) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() => selectTrip(trip)}
                      >
                        {/* Trip header with proper numbering and start/end times */}
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                          <div style={{ flex: 1 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                              <strong style={{ color: '#2c3e50', fontSize: '1.0rem' }}>
                                Trip {trip.tripNumber} of {trip.totalTrips}
                              </strong>
                              <div style={{
                                padding: '4px 8px',
                                backgroundColor: getRiskLevel(trip.behavior_score).color + '15',
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '8px', fontSize: '0.85rem', color: '#6c757d' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                <IonIcon icon={calendarOutline} style={{ fontSize: '0.9rem' }} />
                        ...
                        {selectedTrip?.trip_id === trip.trip_id && (
                          <div style={{ marginTop: '16px', paddingTop: '16px', borderTop: '1px solid #e9ecef' }}>
                            ...
                            {(trip.avg_speed_driving_time_only || trip.driving_time_percentage) && (
                              <div style={{ marginBottom: '16px', padding: '12px', backgroundColor: '#f8f9fa', borderRadius: '6px' }}>
                                <h5 style={{ margin: '0 0 8px 0', color: '#2c3e50', fontSize: '0.9rem', fontWeight: '600' }}>
                                  Enhanced Speed Analysis
                                </h5>
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '12px', fontSize: '0.85rem' }}>
                                  {trip.avg_speed_driving_time_only && (
                                    <div>
                                      <div style={{ color: '#6c757d' }}>Avg Speed (Driving)</div>
                                      <div style={{ fontWeight: '600', color: '#28a745' }}>{safeToFixed(trip.avg_speed_driving_time_only, 1)} mph</div>
                                    </div>
                                  )}
                                  {trip.avg_speed_total_time && (
                                    <div>
                                      <div style={{ color: '#6c757d' }}>Avg Speed (Total)</div>
                                      <div style={{ fontWeight: '600', color: '#6c757d' }}>{safeToFixed(trip.avg_speed_total_time, 1)} mph</div>
                                    </div>
                                  )}
                                  {trip.driving_time_percentage && (
                                    <div>
                                      <div style={{ color: '#6c757d' }}>Driving Time %</div>
                                      <div style={{ fontWeight: '600', color: '#2c3e50' }}>{safeToFixed(trip.driving_time_percentage, 1)}%</div>
                                    </div>
                                  )}
                                  {trip.driving_time_minutes && (
                                    <div>
                                      <div style={{ color: '#6c757d' }}>Actual Driving</div>
                                      <div style={{ fontWeight: '600', color: '#2c3e50' }}>{formatDuration(trip.driving_time_minutes)}</div>
                                    </div>
                                  )}
                                </div>
                                {trip.avg_speed_driving_time_only && trip.avg_speed_total_time && (
                                  <div style={{ marginTop: '8px', fontSize: '0.8rem', color: '#28a745' }}>
                                    Speed improvement: +{safeToFixed(trip.avg_speed_driving_time_only - trip.avg_speed_total_time, 1)} mph when excluding stationary time
                                  </div>
                                )}
                              </div>
                            )}

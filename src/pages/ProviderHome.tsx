...
const ProviderHome: React.FC<ProviderHomeProps> = ({ user, onSignOut }) => {
  ...
  // FIXED: Stable input handler that doesn't cause re-renders
  const handleSearchIdChange = (e: CustomEvent) => {
  ...
  const handleAnalyzeDriver = async () => {
    ...
    try {
      console.log(`🔍 Analyzing driver: ${searchId.trim()}`);
      ...
      <IonContent className="ion-padding">
        <IonCard>
          ...
          <IonCardContent>
            ...
            {/* FIXED: Stable input field */}
            <IonInput
              placeholder="Enter driver user ID (e.g., john-doe)"
              value={searchId}
              onIonInput={handleSearchIdChange}  // FIXED: Use onIonInput instead of onIonChange
              disabled={loading}
              clearInput={true}  // Add clear button
            ...
            <IonButton
              ...
              onClick={handleAnalyzeDriver}
              disabled={!searchId.trim() || loading}
              color={searchId.trim() ? 'primary' : 'medium'}
            >
              {loading ? 'Analyzing Enhanced Data...' : 'Analyze Driver (Enhanced)'}
            </IonButton>
            
            {/* FIXED: Show current search value for debugging */}
            {searchId && (
              <IonText style={{ fontSize: '0.8em', color: 'var(--ion-color-medium)', marginTop: '5px' }}>
                <p>Searching for: "{searchId.trim()}"</p>
              </IonText>
            )}
          </IonCardContent>
        </IonCard>

        {/* Error display - FIXED: Better error styling */}
        {error && (
        ...
        {analytics && (
          <>
            ...
            {/* Rest of your segments remain the same... */}
            {activeSegment === 'detailed' && (
              <IonCard>
                <IonCardHeader>
                  <IonCardTitle>Enhanced Detailed Driving Statistics</IonCardTitle>
